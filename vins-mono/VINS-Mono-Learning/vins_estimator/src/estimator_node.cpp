...
void predict(const sensor_msgs::ImuConstPtr &imu_msg)
{
    ...
    // acc_0相当于a_bk, linear_acceleration相当于a_bk+1
    // gyr_0相当于w_bk, angular_velocity相当于w_bk+1
    // a0 = q_bibk * (a_bk - ba) - gw
    Eigen::Vector3d un_acc_0 = tmp_Q * (acc_0 - tmp_Ba) - estimator.g;
    // w0 = 1/2( (w_bk - b_kg) + (w_bk+1 - b_kg) )
    Eigen::Vector3d un_gyr = 0.5 * (gyr_0 + angular_velocity) - tmp_Bg;
    // q_bibk+1=q_bibkX[1, 1/2wdt]
...
void process()
{
    while (true)
    {
        ...
        {
            // measurement有两个元素，1是imu时间队列，2是对应的当前图像
            //对应这段的img data
