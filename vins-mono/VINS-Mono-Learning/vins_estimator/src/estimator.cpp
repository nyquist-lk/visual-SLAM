...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (frame_count != 0)
    {
        ...
        //采用的是中值积分的传播方式 (P V Q Ba Bg)每个图像时刻的预积分值
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    if(ESTIMATE_EXTRINSIC == 2)//如果没有外参则进行标定
    {
        ...
        if (frame_count != 0)
        {
            ...
            // 利用第i帧到第i+1帧的旋转和imu预积分的旋转不断标定，i从0到n-1
...
bool Estimator::initialStructure()
{
    ...
    //将f_manager中的所有feature（路标点）保存到存有SFMFeature对象的sfm_f中
    Quaterniond Q[frame_count + 1];
    Vector3d T[frame_count + 1];
    map<int, Vector3d> sfm_tracked_points;
    vector<SFMFeature> sfm_f;
    for (auto &it_per_id : f_manager.feature) // 每个特征点
    {
        int imu_j = it_per_id.start_frame - 1; // 当前路标点对应的imu索引号
        SFMFeature tmp_feature; 
        tmp_feature.state = false; // 初始化为false
        tmp_feature.id = it_per_id.feature_id; // 当前路标点索引号
        // 遍历对应当前路标点的特征点，即观察到当前路标点的图像
        for (auto &it_per_frame : it_per_id.feature_per_frame) 
        {
            imu_j++;
            Vector3d pts_j = it_per_frame.point; // 归一化的点
            // <imu_id, <cx, cy>>
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        cv::cv2eigen(t, T_pnp);
        T_pnp = R_pnp * (-T_pnp);
        // 这里转换到 R_wi t_wi(由相机得到的数据，这里的w坐标系原点为参考帧坐标系)
 ...
 *              添加残差，残差项分为4块 先验残差(边缘化残差)+IMU残差+视觉残差+闭环检测残差
