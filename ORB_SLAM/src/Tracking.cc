...
void Tracking::GrabImage(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    // 转换为帧格式 mK为内参矩阵 mDistCoef畸变矩阵
    if(mState==WORKING || mState==LOST)
        // Frame该操作会提取特征点、描述子并分割特征点到图像栅格中
...
void Tracking::FirstInitialization()
{
    ...
    if(mCurrentFrame.mvKeys.size()>100)
    {
        ...
        // 配置初始化器 sigma:1.0 iterations:200
...
bool Tracking::TrackPreviousFrame()
{
    ...
    if(nmatches>=10)
    {
        // Optimize pose with correspondences 基于g2o执行PnP
    ...
    // 执行第二次pnp
    // Optimize pose again with all correspondences pnp
    Optimizer::PoseOptimization(&mCurrentFrame);
...
bool Tracking::TrackLocalMap()
{
    ...
    // Search Local MapPoints 根据限制条件选取局部地图点
