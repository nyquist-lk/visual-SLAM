...
/*
共视图(covisibility graph)：将每个关键帧作为一个节点，若两个关键帧存在一定数量（至少15个）
                            共视地图点则这两个这两个关键帧之间存在一条边，边的权重为共视地图点的数量。
本质图(essential graph)：使用位姿图来校正闭环处的累积误差，由于共视图中边的数量较多，
                        我们创建共视图的子图-本质图。本质图包含生成树、共视地图点数量
                        超过100的边和回环的边
生成树(spanning tree)：ORB-SLAM系统在第一个关键帧开始就维护一个生成树，
                        每个关键帧只与共视点最多的相邻帧产生边。
*/
